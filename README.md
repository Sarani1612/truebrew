...
On small screens, each of the cards will take up the full viewport width and are stacked while they will be shown 2 per row on medium sizes and 3 per row on large and up.
...
- **Checkout:** the checkout view shows an order summary on the left-hand side on medium screens and up (at the top on small and medium screens) and on the right-hand side is the checkout form (bottom on small and medium screens). The form input fields prepopulate with any available data from the User and UserInfo models. At the bottom of the form is the Stripe form which handles payments.
